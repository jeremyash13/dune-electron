...
export default function Volume() {
  ...
  return (
    <div className="flex absolute top-1/2 left-1/2 transform -translate-y-1/2 translate-x-32">
      ...
      <div className="my-auto h-7px relative">
        <input
          className="w-32 absolute h-full volume-slider bg-transparent rounded-full"
          type="range"
          name="volume"
          min="0"
          max="100"
          ...
          defaultValue="90"
          onInput={(e) => {
            store.setVolume(parseInt(e.target.value));
          }}
          style={{
            background: `linear-gradient(to right, rgba(255, 0, 17, .4) 0%, rgba(255, 0, 17, .4) ${store.volume}%, rgba(36, 38, 49, 1) ${store.volume}%, rgba(36, 38, 49, 1) 100%)`,
          }}
        ></input>
      </div>
    </div>

...
export default function PlayPause() {
  ...
  return useObserver(() => (
    <div className="bg-gray-800 mx-6 text-2xl w-12 h-12 rounded-full relative cursor-pointer" onClick={() => {store.togglePlay()}}>
      <div id="PAUSE" className={`${store.isPlaying ? '' : 'hidden'} w-6 h-6 absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2`}>
      <svg
      ...
      <div id="PLAY" className={`${store.isPlaying ? 'hidden' : ''} w-8 h-8 absolute top-1/2 left-1/2 transform -translate-x-3 -translate-y-1/2`}>
        <svg
          className="h-full mx-auto"
          viewBox="0 0 11 14"
          version="1.1"
